...
void refresh_status() {
    if (event_exists(L"Local\\HalfSwordBridge21Running")) {
        ...
        SetWindowTextW(g_status, L"Start failed. Check HalfSwordBridge21.log in this folder.");
        g_start_requested = 0;
    } else {
...
void start_bridge(HWND window) {
    if (event_exists(L"Local\\HalfSwordBridge21Running")) return;
    HANDLE start = OpenEventW(EVENT_MODIFY_STATE, FALSE, L"Local\\HalfSwordBridge21Start");
    if (start) {
        SetEvent(start);
        CloseHandle(start);
...
void stop_bridge() {
    ...
    HANDLE stop = OpenEventW(EVENT_MODIFY_STATE, FALSE, L"Local\\HalfSwordBridge21Stop");
    if (stop) {
        SetEvent(stop);
        CloseHandle(stop);
    }
...
void reset_inputs() {
    HANDLE reset = OpenEventW(EVENT_MODIFY_STATE, FALSE, L"Local\\HalfSwordBridge21Reset");
...
void toggle_recording(HWND window) {
    ...
    else if (event_exists(L"Local\\HalfSwordBridge21Running")) {
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    RegisterClassW(&kind);
    HWND window = CreateWindowExW(0, kind.lpszClassName, L"Half Sword joiner controls 1.2",
