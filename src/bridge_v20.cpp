...
bool resolve_native_input() {
    ...
    if (!readable(world, 8) || !*world) return false;
    auto *instance = reinterpret_cast<void **>(reinterpret_cast<std::uintptr_t>(*world) + 0x1D8);
    if (!readable(instance, 8) || !*instance) return false;
    auto *players = reinterpret_cast<Array *>(reinterpret_cast<std::uintptr_t>(*instance) + 0x38);
    if (!readable(players, sizeof(Array)) || players->count != 2 || !readable(players->data, 16)) return false;
    ...
    if (!g_native_keys_ready) {
        ...
        if (!readable(objects, sizeof(ObjectArray)) || objects->num_chunks < 1 ||
            ...
            !readable(objects->chunks, objects->num_chunks * sizeof(void *))) return false;
        ...
        for (int i = 0; i < objects->num_elements; ++i) {
            ...
            for (int n = 0; n < actions->count; ++n) {
                ...
                if (wcscmp(key_name, L"LeftMouseButton") == 0) left = mapping->key;
                if (wcscmp(key_name, L"RightMouseButton") == 0) right = mapping->key;
                if (wcscmp(key_name, L"SpaceBar") == 0) space = mapping->key;
                if (wcscmp(key_name, L"X") == 0) x_key = mapping->key;
                for (int key_index = 0; key_index < kHeldKeyCount; ++key_index)
                    if (wcscmp(key_name, kHeldKeyNames[key_index]) == 0)
                        held_keys[key_index] = mapping->key;
            }
            for (int n = 0; n < axes->count; ++n) {
                auto *mapping = static_cast<AxisMapping *>(axes->data) + n;
                wchar_t key_name[128]{}; String key_text{key_name, 0, 128};
                append(&mapping->key.name, key_text);
                if (wcscmp(key_name, L"MouseX") == 0) mouse_x = mapping->key;
                if (wcscmp(key_name, L"MouseY") == 0) mouse_y = mapping->key;
        ...
        if (!left.name.comparison || !right.name.comparison ||
            !readable(reinterpret_cast<void *>(left.details), 8) ||
            !readable(reinterpret_cast<void *>(right.details), 8) ||
            (space.name.comparison && !readable(reinterpret_cast<void *>(space.details), 8)) ||
            (x_key.name.comparison && !readable(reinterpret_cast<void *>(x_key.details), 8))) return false;
        for (int key_index = 0; key_index < kHeldKeyCount; ++key_index)
            if (!held_keys[key_index].name.comparison ||
                !readable(reinterpret_cast<void *>(held_keys[key_index].details), 8)) return false;
        g_left_key = left; g_right_key = right; g_space_key = space; g_x_key = x_key;
        if (mouse_x.name.comparison && mouse_y.name.comparison &&
            readable(reinterpret_cast<void *>(mouse_x.details), 8) &&
            readable(reinterpret_cast<void *>(mouse_y.details), 8)) {
...
bool held_key_button(int index, bool down) {
    if (index < 0 || index >= kHeldKeyCount || !resolve_native_input()) return false;
...
void handle_batch() {
    ...
    for (uint32 i = 0; i < g_count; ++i) {
        ...
        if (event.m_eType == k_ERemotePlayInputKeyDown || event.m_eType == k_ERemotePlayInputKeyUp ||
            ...
            if (binding < 0) { ++g_dropped; continue; }
        ...
        if (event.m_eType == k_ERemotePlayInputMouseMotion) {
            ...
            if (held_index >= 0) {
                if (held_key_button(held_index, phase == 0)) ++g_keys;
                else ++g_dropped;
                continue;
            }
            int function = -1;
